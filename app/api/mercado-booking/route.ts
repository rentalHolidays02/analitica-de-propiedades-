...
import { borrar, insertar } from "@/lib/db";
import { propiedadesActivas } from "@/lib/consultas";
import { preciosBooking } from "@/lib/mercado";
...
export async function POST() {
  ...
  for (const [zona, hueco] of objetivo) {
    try {
      ...
      const vecinos = await preciosBooking(zona, checkin, checkout);

      // Repetir la consulta el mismo dia no debe duplicar filas: la mediana se
      // torceria. Se reemplaza lo que ya hubiera de Booking para esa zona y fecha.
      await borrar("competencia",
        `zona=eq.${encodeURIComponent(zona)}&fecha_estancia=eq.${checkin}&fuente=eq.booking`);
  ...
  return NextResponse.json({ zonas: resumen.length, resumen });
}
