...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Un fallo de red (timeout, DNS) al llamar a Groq no debe tumbar el route
    // handler: sin este catch, Next devuelve su pagina de error en HTML y el
    // cliente revienta al hacer r.json() ("Unexpected token '<'").
    return NextResponse.json({ error: e instanceof Error ? e.message : "Fallo al conectar con Groq" }, { status: 502 });
  }
}
